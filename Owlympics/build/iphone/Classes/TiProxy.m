...
//Rare exceptions to indicate a bug in the _owlympics code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_owlympics = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_owlympics]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.owlympics.module"
